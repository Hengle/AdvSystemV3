...
namespace SpriteDicing
{
    ...
    public class DicedSpriteAtlasEditor : Editor
    {
        ...
        private SerializedProperty characterTermProperty;
        ...
        {
            characterTermProperty = serializedObject.FindProperty("characterTerm");
        ...
        public override void OnInspectorGUI ()
        {
            ...
            EditorGUILayout.PropertyField(characterTermProperty, true);
            ...
            // EditorGUILayout.LabelField(new GUIContent("Atlas 新舊名稱映射 (可使舊資源的Reference保存)"));
            // EditorGUILayout.Space();
            // RefferenceNumberProp.intValue = EditorGUILayout.IntField(new GUIContent("Number"), RefferenceNumberProp.intValue);
            // EditorGUILayout.Space();
            // int totalCount = RefferenceNumberProp.intValue;
            // for (int i = 0; i < totalCount; i++)
            // {
            //     if(oldNamesProp.arraySize != totalCount){
            //         oldNamesProp.arraySize = totalCount;
            //     }
            //     if(newNamesProp.arraySize != totalCount){
            //         newNamesProp.arraySize = totalCount;
            //     }

            //     using (new EditorGUILayout.HorizontalScope())
            //     {
            //         oldNamesProp.GetArrayElementAtIndex(i).stringValue = EditorGUILayout.TextField("Old Name: ", oldNamesProp.GetArrayElementAtIndex(i).stringValue);
            //         newNamesProp.GetArrayElementAtIndex(i).stringValue = EditorGUILayout.TextField("New Name: ", newNamesProp.GetArrayElementAtIndex(i).stringValue);
            //     }
            // }
            EditorGUILayout.Space();
            EditorGUILayout.Space();
            //if (GUILayout.Button(new GUIContent("Remove AtlasTexture-NullRefference Sub-Assets"), EditorStyles.miniButtonLeft))
            //    RemoveNullRefAssets();
            if (GUILayout.Button(new GUIContent("Reference child Atlas texture with this"), EditorStyles.miniButtonMid))
                ReferenceChildAtlas();
